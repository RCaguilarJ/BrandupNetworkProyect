...
const SYSTEM_PROFILES: SystemProfile[] = [
  {
    id: 'billing-profile',
    profile: 'COBRANZA',
    description: 'COBRANZA DE FACTURAS PENDIENTES',
    createdAt: '07/07/2022',
    groupedRoles: ['cobranza'],
  },
  {
    id: 'technical-profile',
    profile: 'TECNICO',
    description: 'CLIENTES, TICKET Y COBRANZA, CON RESTRICCIONES',
    createdAt: '07/07/2022',
    groupedRoles: ['tecnico', 'soporte'],
  },
  {
    id: 'administrator-profile',
    profile: 'ADMINISTRADOR',
    description: 'ACCESOS A TODOS LOS MODULOS',
    createdAt: '07/07/2022',
    groupedRoles: ['super_admin', 'isp_admin'],
  },
];
...
export default function RolesManagement() {
  ...
  /**
   * La pantalla expone el catálogo de perfiles del sistema a partir de los roles
   * reales definidos en el código. Backend deberá reemplazar este adapter por su
   * propio listado persistido cuando exista la gestión formal de perfiles.
   */
  const records = useMemo<RoleManagementRecord[]>(() => {
    return SYSTEM_PROFILES.map((profile) => ({
      id: profile.id,
      profile: profile.profile,
      description: profile.description,
      associatedUsers: user && profile.groupedRoles.includes(user.role) ? 1 : 0,
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((record) =>
      `${record.profile} ${record.description}`.toLowerCase().includes(searchTerm.toLowerCase()),
    );
  }, [records, searchTerm]);
  ...
  function handleRefresh() {
    toast.success('Listado de perfiles actualizado');
  }

  function handleCreateRole() {
    toast.info('La creación de perfiles queda lista para integrarse con backend.');
  }

  return (
    <div className="min-h-[calc(100vh-4rem)] bg-[#d3dce7] px-[30px] pb-6 pt-[18px]">
      <div className="mb-4 flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
        <h1 className="text-[26px] font-normal leading-none text-[#1f2933]">Gestión de Perfiles</h1>
        ...
        <div className="flex items-center gap-2 pt-[3px] text-[12px] text-[#1f2933]">
          ...
          <span>Ajustes</span>
          <span>/</span>
          <span className="text-[#1bc3dc]">Perfiles</span>
        </div>
      </div>
      ...
      <section className="overflow-hidden rounded-[2px] border border-[#d7dfe8] bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-[#d7dfe8] px-4 py-3">
          <p className="text-[14px] text-[#1f2933]">Lista de perfiles</p>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title="Expandir"
              aria-label="Expandir listado de perfiles"
            >
            ...
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full border border-[#d7dfe8] text-[#4b5563] hover:bg-[#f8fafc]"
              title="Actualizar"
              aria-label="Actualizar listado de perfiles"
            >
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border border-[#d7dfe8]">
                  ...
                  <th className="w-[20%] border-r border-[#d7dfe8] px-3 py-[9px] text-left text-[14px] font-normal text-[#111827]">PERFIL</th>
                  <th className="w-[47%] border-r border-[#d7dfe8] px-3 py-[9px] text-left text-[14px] font-normal text-[#111827]">DESCRIPCIÓN</th>
                  <th className="w-[9%] border-r border-[#d7dfe8] px-3 py-[9px] text-center text-[14px] font-normal text-[#111827]">USUARIOS ASOCIADOS</th>
                  <th className="w-[8%] border-r border-[#d7dfe8] px-3 py-[9px] text-left text-[14px] font-normal text-[#111827]">CREADO</th>
                  <th className="w-[6%] border-r border-[#d7dfe8] px-3 py-[9px] text-center text-[14px] font-normal text-[#111827]">ESTADO</th>
              ...
              <tbody>
                {paginatedRecords.length === 0 ? (
                  <tr className="border border-t-0 border-[#d7dfe8]">
                    <td colSpan={7} className="px-3 py-[18px] text-center text-[14px] text-[#374151]">
                      No hay información
                    </td>
                  </tr>
                ) : (
                  paginatedRecords.map((record, index) => (
                    <tr key={record.id} className="border border-t-0 border-[#d7dfe8] text-[14px] text-[#111827]">
                      ...
                      <td className="border-r border-[#d7dfe8] px-3 py-[10px]">{record.description}</td>
                      <td className="border-r border-[#d7dfe8] px-3 py-[10px] text-center">
                        <span className="inline-flex min-w-[22px] items-center justify-center rounded-full bg-[#f59e0b] px-2 py-[1px] text-[12px] font-semibold text-white">
                          {record.associatedUsers}
                        </span>
                      </td>
                      <td className="border-r border-[#d7dfe8] px-3 py-[10px]">{record.createdAt}</td>
                      <td className="border-r border-[#d7dfe8] px-3 py-[10px] text-center">
                      ...
                      <td className="px-3 py-[10px]">
                        <div className="flex items-center justify-center gap-2">
                          <button
                            ...
                            onClick={() => toast.info('Edición de perfiles lista para conectarse con backend.')}
                            aria-label={`Editar perfil ${record.profile}`}
                          >
                          ...
                          <button
                            ...
                            onClick={() => toast.info('Matriz de permisos lista para integrarse.')}
                            aria-label={`Ver permisos del perfil ${record.profile}`}
                          >
