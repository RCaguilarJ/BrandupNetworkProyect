...
type MonitoringColumn = {
  key: MonitoringColumnKey;
  header: string;
  label: string;
  width?: string;
  getValue: (row: MonitoringRow) => string;
};

const MONITORING_COLUMNS: MonitoringColumn[] = [
  { key: 'id', header: 'ID', label: 'ID', width: '70px', getValue: (row) => row.id },
  { key: 'nombre', header: 'NOMBRE', label: 'Nombre', getValue: (row) => row.nombre },
  { key: 'equipo', header: 'EQUIPO', label: 'Equipo', width: '210px', getValue: (row) => row.equipo },
  { key: 'ip', header: 'IP', label: 'IP', width: '120px', getValue: (row) => row.ip },
  { key: 'estado', header: 'ESTADO', label: 'Estado', width: '170px', getValue: (row) => row.estado },
  { key: 'online', header: 'ONLINE', label: 'Online', width: '170px', getValue: (row) => row.online },
  { key: 'activos', header: 'ACTIVOS', label: 'Activos', width: '170px', getValue: (row) => row.activos },
  {
    key: 'suspendidos',
    header: 'SUSPENDIDOS',
    label: 'Suspendidos',
    width: '190px',
    getValue: (row) => row.suspendidos,
  },
];
...
export default function NetworkMonitoring() {
  ...
  const [visibleColumnKeys, setVisibleColumnKeys] = useState<MonitoringColumnKey[]>(
    MONITORING_COLUMNS.map((column) => column.key),
  );
  const [rows, setRows] = useState<MonitoringRow[]>(() => readStoredRows('network-monitoring-rows', []));
  ...
  useEffect(() => {
    window.localStorage.setItem('network-monitoring-rows', JSON.stringify(rows));
  }, [rows]);

  const visibleColumns = useMemo(() => {
    const filtered = MONITORING_COLUMNS.filter((column) => visibleColumnKeys.includes(column.key));
    return filtered.length > 0 ? filtered : MONITORING_COLUMNS;
  }, [visibleColumnKeys]);
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) =>
      MONITORING_COLUMNS.some((column) => column.getValue(row).toLowerCase().includes(query)),
    );
  }, [rows, searchTerm]);
  ...
  const toggleColumn = (columnKey: MonitoringColumnKey) => {
  ...
  const saveRow = () => {
    if (!form.nombre.trim() || !form.equipo.trim() || !form.ip.trim()) {
    ...
    setRows((current) => [
      ...current,
      {
        ...
        activos: form.activos.trim(),
        suspendidos: form.suspendidos.trim(),
      },
    ]);
    dialog.closeDialog();
    setCurrentPage(1);
  ...
  const handleDatasetPrint = () => {
    openDatasetPrintPreview('Monitoreo', exportRows, true, exportHeaders);
    setExportMenuOpen(false);
  };
  ...
  const handleDatasetExportCsv = () => {
    ...
    downloadBlob('monitoreo.csv', `\uFEFF${lines.join('\n')}`, 'text/csv;charset=utf-8;');
    setExportMenuOpen(false);
  };
