...
import { type ZoneManagementRecord } from '../types';
import {
  ChevronLeft,
  ChevronRight,
  Ellipsis,
...
const SYSTEM_ZONES: ZoneManagementRecord[] = [];
...
export default function ZonesManagement() {
  ...
  /**
   * La referencia aprobada para zonas muestra el listado vacio.
   ...
   */
  const records = useMemo(() => SYSTEM_ZONES, []);
  ...
  function handleRefresh() {
    toast.success('Listado de zonas actualizado');
  }

  function handleCreateZone() {
    toast.info('La creacion de zonas queda lista para integrarse con backend.');
  }
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-[#d3dce7] px-[30px] pb-6 pt-[18px]">
      <div className="mb-4 flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
        ...
        <div className="flex items-center gap-2 pt-[3px] text-[12px] text-[#1f2933]">
          ...
          <span>Ajustes</span>
          <span>/</span>
          <span className="text-[#1bc3dc]">Zonas</span>
        </div>
      </div>
      ...
      <section className="overflow-hidden rounded-[2px] border border-[#d7dfe8] bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-[#d7dfe8] px-4 py-3">
          <p className="text-[14px] text-[#1f2933]">Lista de Zonas</p>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              title="Expandir"
              aria-label="Expandir listado de zonas"
            >
            ...
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full border border-[#d7dfe8] text-[#4b5563] hover:bg-[#f8fafc]"
              title="Actualizar"
              aria-label="Actualizar listado de zonas"
            >
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border border-[#d7dfe8]">
                  <th className="w-[14%] border-r border-[#d7dfe8] px-3 py-[9px] text-left text-[14px] font-normal text-[#111827]">
                    N°
                  </th>
                  <th className="w-[33%] border-r border-[#d7dfe8] px-3 py-[9px] text-left text-[14px] font-normal text-[#111827]">
                    ZONA
                  </th>
                  <th className="w-[29%] border-r border-[#d7dfe8] px-3 py-[9px] text-center text-[14px] font-normal text-[#111827]">
                    CREADO
                  </th>
